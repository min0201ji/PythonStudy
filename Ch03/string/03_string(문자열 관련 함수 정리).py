...
print(crawling.find('f')) #문자의 위치
print(crawling.find('parsing')) # -1 = 없음을 의미
print(crawling.find('a')) # 문자가 여러번 있어도, 처음 찾은 문자의 위치를 반환

# index(): 문자열 내에서 특정 문자열의 시작 위치를 반환
crawling = 'Data crawling is fun'
print(crawling.index('fun')) #문자열의 시작 위치 반환
print(crawling.index('f')) #문자의 위치
#print(crawling.index('parsing')) # 임마는 에러남(ValueError:substring not found)
print(crawling.index('a')) # 문자가 여러번 있어도, 처음 찾은 문자의 위치를 반환

# split(): #구분자를 기준으로 문자열을 나눔
# 나눈 결과를 리스트로 반환
# 구분자: 기본 공백
# 구분자 지정: "-",":",","... 어떤 문자든지 구분자로 지정 가능하다

string = 'Python Ptogramming'
...
# 문자열과 리스트의 차이
for c in colors_split: # list출력 - 각 요소 출력
    print(c)
...
# 전체문자열.replace(찾고자 하는 문자열, 변경할 문자열)
# 찾는 문자열이 존재하면 변경된 문자열을 반환한다
...
#이 문장 수행 후 course 변수의 값은 변경 되는가? -> 원본을 변경하지 않는다
#replace는 원본은 변경 X
...
#capitalize(): 문장의 첫번째 문자를 대문자로, title(): 단어의 첫 글자를 대문자로
print('====================')
...
ID = input('ID 입력: ')
# 문자 또는 숫자가 아닌경우:
if not(ID.isalnum()): #문자 또는 숫자 또는 문자+숫자
    print('문자와 숫자만 사용할 수 있습니다.')

# 스페이스 여부 확인(space로만 이루어져 있는가?)
print(' '.isspace()) # 스페이스(공간)가 한 개인 경우
print(' c'.isspace()) # 다은 문자와 혼합
print('  '.isspace()) # 스페이스가 두 개 이상
