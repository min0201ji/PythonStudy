...
for i in range(0,len(split_str_data)):
    ...
    # } 괄호를 기준으로 분리해서 왼쪽 숫자만 추출2
    #누적 합산
    total += int(temp)
print()
print('추출된 숫자의 합은 %d 입니다' %total)
print(type(split_str_data[0].split(':')[1].split('}')[0]))
print(split_str_data[0].split(':')[1].split('}')[0])
print(split_str_data[2].split(':')[1].split('}')[0])
