...
# iterable : 각각의 요소를 하나씩 반환할 수 있는 객체 / for문으로 반복해서 출이 가능 자료형
# 리스트, 튜플, 집합, 딕셔너리
...
# enumerate 객체반환: <enumerate object at 0x7f8c480d2d80>
...
print('===filter()===')
### iterable -> 반복가능한 문자열(리스트,튜플,...)
# filter(function(이름만 씀),iterable): 반복 가능한 자료형 요소들이
...
print(filter(positive,[0,-1,5,-7,10])) #<filter object at 0x7f97281deee0>
...
print('===map===')
# map(function이름,iterable): iterable 각 요소가 함수 function에 의해 수행된 결과 반

def increase(x):
    return x + 1

print(map(increase,[1,2,3,4,5])) #<map object at 0x7fe8c81e2760>
print(list(map(increase,[1,2,3,4,5]))) # [2, 3, 4, 5, 6]

print('===open()===')
# 외부 파일을 사용하기 위해서 접근 경로를 생성하는 함수(파일이 없으면 만들고, 있으면 해당 파일을 열어준다.)
file_write = open('my_file.txt','w')

print('===round()===')
# 실수를 반올림 하여 반환하는 함숨
# round(number[,ndigits]): ndigits - 소수 이하 자리수₩( []-> 생략 가능!)
print(round(3.141592,3)) #3.142

print('===zip===')
#zip(iterable,iterable1...): 각 iterable에서 동일한 인덱스의 요소를 추출하여 튜플 형태로 묶어 반환
print(zip([1,2,3],[4,5,6])) # <zip object at 0x7fd9200eebc0>
print(list(zip([1,2,3],[4,5,6]))) # [(1, 4), (2, 5), (3, 6)]
print(list(zip([1,2,3],[4,5]))) #[(1, 4), (2, 5)]
print(list(zip('123','abc'))) #[('1', 'a'), ('2', 'b'), ('3', 'c')]

# zip 함수를 사용해서 튜플로부터 딕셔너리를 생성하는 예제
