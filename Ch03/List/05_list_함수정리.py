...
a = [1,2,3]
a.extend([4,5])
print('a 리스트:',a) # a 리스트: [1,2,3,4,5]

b = [1,2,3]
b.append([4,5]) # b 리스트: [1,2,3[4,5]]
print('b 리스트:' ,b)
# extend는 원소가 추가되면서 리스트가 확장됨 = 하나의 리스트
# append는 추가 원소가 리스트면 하위 리스트로 추가 됨 - 리스트안의 리스트

c = [1,2,3]
c.insert(len(c),[4,5])
print('c 리스트: ', c) # c 리스트: [1,2,3,[4,5]]
...
# 복합 자료형인 경우 max, min 함수 사용 불가!
#print(min(n_char))
#print(max(n_char))
#-> ypeError: '<' not supported between instances of 'str' and 'int'
# str과 int사이에서는 불가능

# in/not in (포함여부 파단 후 True/False로 반환)
num = [1,2,3,4,5]
...
# 리스트 일치 검사 (두개의 리스트가 동일한지 검사) #
# 비교 연산자를 사용해서 2개의 리스트 비교 (==,!=,>,>=,<,<=)
# 첫번째 요소부터 비교 시작
# 첫번째 요소의 비교에서 결과가 False면 더 이상 비교 하지 않고
# 첫번째 요소가 동일하면 두번째 요소 비교...
# 리스트 안의 모든 요소 비교 결과가 True면 전체 결과: True
